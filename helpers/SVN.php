    ...
    class SVN {
        ...
        public function revisions() {
            ...
            if(isset($xml->entry)) {
                ...
                $revisionsMarkup .= view("info.html", array("message" => "Current revision: ".$attributes->revision));
            }

            // Getting latest commits
            $shell = new Shell("cd ".$this->destination." && svn log -v --xml --with-all-revprops -l 10 --username ".$this->app->user." --password ".$this->app->pass);
            $result = $shell->toString();
            ...
            if(isset($xml->logentry)) {
                foreach($xml->logentry as $entry) {
                    ...
                    $revisionsMarkup .= view("revision.html", array(
                        ...
                        "action-release" => view("form.command.html", array(
                            "cmd" => isset($attributes->revision) ? "cd ".$this->destination." && svn update -r ".$attributes->revision." --username ".$this->app->user." --password ".$this->app->pass : "",
